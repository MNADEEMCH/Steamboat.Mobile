...
namespace Steamboat.Mobile.ViewModels
{
    public class MenuViewModel : ViewModelBase
    {
        ...
        private MainViewModelInitParameter _initParameter;
        ...
        public override Task InitializeAsync(object parameter)
        {
            ...
            _initParameter = parameter as MainViewModelInitParameter;
            InitMenuItems(_initParameter);
            _applicationManager.OnNotification += OnNotificationHandler;
            _applicationManager.OnChangeMenuOrder += OnChangeMenuOrder;
        ...
        private void OnChangeMenuOrder(object sender, EventArgs e)
        {
            if (!MenuItems[0].Title.Equals("Photojournaling"))
            {
                MenuItems = new ObservableCollection<Models.Menu.MenuItem>();
                _initParameter.NavigatingToDispositionStep = false;
                _initParameter.NutritionPlan.IsActivePhase = true;
                InitMenuItems(_initParameter);
        ...
        private void InitMenuItems(MainViewModelInitParameter initParameter)
        {
            ...
            if (initParameter.NutritionPlan.IsEnabled)
            {
                var position = initParameter.NutritionPlan.IsActivePhase ? 0 : 1;
                MenuItems.Insert(position, new Models.Menu.MenuItem
                {
                    Title = "Photojournaling",
                    IsSelected = initParameter.NutritionPlan.IsActivePhase,
            ...
            {
                Title = "Messages",
                IsSelected = !isDispositionStepSelected && !initParameter.NutritionPlan.IsActivePhase,
            ...
            if (GetMessagesMenuItem().IsSelected)
                GetMessagesMenuItem().NotificationsCount = 0;
            HandleNotificationsCount();
        }
